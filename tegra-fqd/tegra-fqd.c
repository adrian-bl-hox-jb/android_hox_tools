...
static void update_freq() {
	...
	while( (dentry = readdir(dfd)) != NULL ) {
		...
		if(!strcmp(T_AUDIO_ON, dentry->d_name))
			minfreq = MINFREQ_AUDIO;
		if(!strcmp(T_A2DP_ON, dentry->d_name))
			minfreq = MINFREQ_A2DP;
	}
	closedir(dfd);

	sysfs_write("/sys/devices/system/cpu/cpu0/cpufreq/scaling_min_freq",         minfreq);
	sysfs_write("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq",         on ? pp[0] : pp[1]);
	sysfs_write("/sys/module/cpu_tegra/parameters/cpu_user_cap",                 on ? pp[0] : pp[1]); /* same as max_freq */
